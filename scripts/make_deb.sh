...
fpm -s dir -t deb \
    ...
    --chdir "$STAGING_DIR" \
    --package "$OUT_DIR/tinyllamax_${VERSION}_all.deb" \
    . <<'POSTINST'
#!/bin/sh
set -e
...
echo "DEB package created: $OUT_DIR/tinyllamax_${VERSION}_all.deb"
dpkg -c "$OUT_DIR/tinyllamax_${VERSION}_all.deb" | head -20
